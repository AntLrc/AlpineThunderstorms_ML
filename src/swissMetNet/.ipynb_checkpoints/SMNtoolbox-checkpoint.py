...
def SpectralStationClustering(data, **kwargs):
    ...
    """
    # To modify to allow data to be the affinity matrix, consistant with sklearn
    stations = data.station.values
    nb_of_stations = len(stations)
    affinities = kwargs.get("affinities", np.array([[xr.corr(data.isel(station=i), data.isel(station=j), dim = "time").values for i in range(nb_of_stations)] for j in range(nb_of_stations)]))
    ...
    result.where(~nansmatrix, drop = True)
    affinities = affinities[~nansmatrix]
